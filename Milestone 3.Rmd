...
```{r}
data1 <- read.csv("results_acled_aer (2).do")
```
...
#The variables above are different conditions that the authors account for when examining the relationship between ethnic partioning and future cooperation efforts. 
...
#Making a regression table, with a raw coefficent and a standard error.
...
#Calculating a table of summary statistics.
#Showing the mean for regions where the population was not equal to 0 in 1960.
...
#Using the same format from above but instead it's showing linear probability rather than Maximum Likelihood.

** Mean Dependent Variable  
  tabstat  $allvars   if pop60!=0 & borderdist1<median_bd, stats(mean )  
  
#Producing a table of sumamry statistics.
#Finding the mean for a population in 1960 that is not equivalent to 0 and has a border distribution that is less than the median of all regions' border distributions.
...
#These are the variables that will be used in this Panel

** Panel A. All Country-Ethnic Homelands
*********************************************************************************************************************
tabstat $acled if pop60!=0 , stats(n mean sd p50 min p90 max) col(stats)

#Calculating a table of summary statistics(counting finding the mean, and standard deviation, the 50th percentile, the 90th percentile, and the maximum) for showing armed conflict. 

tabstat $acledmore if pop60!=0 , stats(n mean sd p50 min p90 max) col(stats)

#Calculating a table of summary statistics(counting finding the mean, and standard deviation, the 50th percentile, the 90th percentile, and the maximum) for showing armed conflict. 
#I can't find information in regards to the difference between these two variables. I'm assuming it's in their other replication files, but still can't open them right now.
...
#Calculating a table of summary statistics if the population of 1960 is not one and the border distribution is less than the median. (counting finding the mean, and standard deviation, the 50th percentile, the 99th percentile, and the maximum) for data from the armed conflict location and event project.

tabstat $acledmore if pop60!=0 & borderdist1<median_bd  , stats(n mean sd p50 min p99 max) col(stats)

#Doing the same thing here, but for another variable 
...
#New variables that will be used in future calculations.
...
#Calulating a regression, showing unconditional estimates and likelihoods with a logarithmic function.
#Starring coefficents that are 0.1,0.005, or 0.01
#Variables represent different country observations
#Estimates here are experiencing a fixed effect
...
#Same code, but with different vabriables. 
...
#Making a statistic regression table and calculating the r squared. Marking a star on coefficents that are 0.1, 0.05, and 0.01.
...
#Using the same code as above, but with new variables.
...
#Calculating the estimates of deadly events in nations with split ethnic groups. Calculating the logarithmic function of the probabilties and the adjusted R-2.
...
#This is the same code from table 3, but uses new variables that are more specific to what type of deadly conflict has occurred. This includes riots, battles, protests, and violence against civilians.
...
#Panel B uses similiar code however it shows a linear probability estimate rather than a negative binomial estimate.
...
#This format is similiar to previous code chunks that we've seen. The variables in this table are indicating actors of ethnic violence. Here we are looking at the adjusted r squared and are only looking at those observations where there is a an ethnic split. 
...
#The variables in this panel represent the statistics of each type of civil conflict to ethnic partioning. These statisitics represent state conflict and this code is looking at a log regression of likelihood.
...
#Creating a regression table for variables showing one-sided violence against civilians
...
#Creating a regression table for variables of non-state conflict.
